...
class tokokoo_flickr extends WP_Widget {
	...
	function __construct() {
		...
		$widget_ops = array( 
			...
			'description' => __( 'A custom widget to display photo galleries from your Flickr account.', 'tokokoo' ) 
		);
		...
		parent::__construct( 'tokokoo_flickr_widget', __( 'Tokokoo - Flickr Photostream', 'tokokoo' ), $widget_ops, $control_ops );
	...
	function widget( $args, $instance ) {
		...
		$after_list = '</ul>';
		$default_title = __( 'Untitled Flickr photo', 'tokokoo' );
	...
	?>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'num_items' ); ?>"><?php _e( 'Display', 'tokokoo'); ?></label>
			...
			<select onchange="javascript: toggleSource('<?php echo $this->get_field_id( 'type' ); ?>');" name="<?php echo $this->get_field_name( 'type' ); ?>" id="<?php echo $this->get_field_id( 'type' ); ?>">
				<option <?php selected( $type, 'user' ); ?> value="user"><?php _e( 'User', 'tokokoo' ); ?></option>
				<option <?php selected( $type, 'set' ); ?> value="set"><?php _e( 'Set', 'tokokoo' ); ?></option>
				<option <?php selected( $type, 'favorite' ); ?> value="favorite"><?php _e( 'Favorite', 'tokokoo' ); ?></option>
				<option <?php selected( $type, 'group' ); ?> value="group"><?php _e( 'Group', 'tokokoo' ); ?></option>
				<option <?php selected( $type, 'public' ); ?> value="public"><?php _e( 'Community', 'tokokoo' ); ?></option>
			</select>
			<?php _e( 'photos', 'tokokoo' ); ?>.
		</p>
		<p class="id_parent">
			<label for="<?php echo $this->get_field_id( 'id' ); ?>"><?php _e( 'User or Group ID', 'tokokoo' ); ?></label>
			<input class="widefat" name="<?php echo $this->get_field_name( 'id' ); ?>" type="text" id="<?php echo $this->get_field_id( 'id' ); ?>" value="<?php echo $id; ?>" /><br />
			<small><a href="<?php echo esc_url( 'http://idgettr.com/' ); ?>" rel="external" target="_blank"><?php _e( 'What is my ID ?', 'tokokoo' );?></a></small>
		</p>
		<p class="set_parent">
			<label for="<?php echo $this->get_field_id( 'set' ); ?>"><?php _e( 'Set ID', 'tokokoo' ); ?></label>
			...
			<small><?php _e( 'Use number from the set url', 'tokokoo' ); ?></small>
		</p>
		<p class="tags_parent">
			<label for="<?php echo $this->get_field_id( 'tags' ); ?>"><?php _e( 'Tags (optional)', 'tokokoo' ); ?></label>
			...
			<small><?php _e( 'Comma separated, no spaces', 'tokokoo' ); ?></small>
		</p>
		<p>
	...
	function getRSS($settings) {
		...
		elseif ($settings['type'] == "public" || $settings['type'] == "community") { $rss_url = 'http://api.flickr.com/services/feeds/photos_public.gne?tags=' . $settings['tags'] . '&format=rss_200'; }
			...
			print __( '<strong>No "type" parameter has been setup. Check your settings, or provide the parameter as an argument.</strong>', 'tokokoo' );
		}
		...
		if ( $feed->error() )
			printf( __( 'There was an error while connecting to Feed server, please, try again!', 'tokokoo' ) );
