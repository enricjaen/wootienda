...
class tokokoo_twitter extends WP_Widget {
	...
	function __construct() {
		...
		$widget_ops = array( 
			...
			'description' => __( 'A custom widget to display your recent tweets. Work with new Twitter API.', 'tokokoo' ) 
		);
		...
		parent::__construct( 'tokokoo_twitter_widget', __( 'Tokokoo - Recent Tweets', 'tokokoo' ), $widget_ops, $control_ops );
	...
	function widget( $args, $instance ) {
		...
		if( empty( $instance['consumer_key'] ) || empty( $instance['consumer_secret'] ) || empty( $instance['screen_name'] ) ){
			echo __( 'Please fill all widget settings.', 'tokokoo' ) . $after_widget;

			return;
		}

		if( ! require_once( trailingslashit( TOKOKOO_CLASS ) . 'class-wp-twitter-api.php' ) ) { 
			echo __( 'Couldn\'t find the required class.', 'tokokoo' ) . $after_widget;
	...
	?>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'screen_name' ); ?>"><?php _e( 'Screen Name:', 'tokokoo' ); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'consumer_key' ); ?>"><?php _e( 'Consumer Key:', 'tokokoo' ); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'consumer_secret' ); ?>"><?php _e( 'Consumer Secret:', 'tokokoo' ); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'num_tweets' ); ?>"><?php _e( 'Number of Tweets:', 'tokokoo' ); ?></label>
		...
		<p><label for="<?php echo $this->get_field_id('show_avatar') ?>">
			...
			<?php _e('Show avatar image', 'tokokoo') ?></label>
		</p>
		<p><label for="<?php echo $this->get_field_id('show_follow') ?>">
			...
			<?php _e('Show follow button', 'tokokoo') ?></label>
		</p>
		<?php
	}
	...
	function relative_time( $time ) {
		...
		if( is_numeric( $diff ) && $diff > 0 ) {
			...
			if($diff < 3) return __( 'right now', 'tokokoo' );
			// if less than 1 minute
			if($diff < $minute) return floor($diff) . __( ' seconds ago', 'tokokoo' );
			// if less than 2 minutes
			if($diff < $minute * 2) return __( 'about 1 minute ago', 'tokokoo' );
			// if less than 1 hour
			if($diff < $hour) return floor($diff / $minute) . __( ' minutes ago', 'tokokoo' );
			// if less than 2 hours
			if($diff < $hour * 2) return __( 'about 1 hour ago', 'tokokoo' );
			// if less than 1 day
			if($diff < $day) return floor($diff / $hour) . __( ' hours ago', 'tokokoo' );
			// if more then 1 day, but less then 2 days
			if($diff > $day && $diff < $day * 2) return __( 'yesterday', 'tokokoo' );
			// if less than 1 year
			if($diff < $day * 365) return floor($diff / $day) . __( ' days ago', 'tokokoo' );
			// else return over a year ago
			return __( 'over a year ago', 'tokokoo' );
		}
	}
