...
function tokokoo_additional_menu() {
	...
	echo '<nav class="additional-menu" role="navigation">';
		if ( ! is_user_logged_in() ) {
			echo '<a class="nav-account" href="' . esc_url( wp_login_url( home_url() ) ) . '">' . __( 'Login', 'tokokoo' ) . '</a>';
			echo '<a class="nav-reg" href="' . esc_url( site_url( 'wp-login.php?action=register' ) ) . '">' . __( 'Register', 'tokokoo' ) . '</a>';
		} else {
			echo '<a class="nav-logs" href="' . esc_url( wp_logout_url( home_url() ) ) . '">' . __( 'Logout', 'tokokoo' ) . '</a>';
	...
	<div class="footer-text">
		<p class="copyright">
			...
			<?php if ( $footer_text == false ) { 
				printf( __( '- Theme by <a href="%1$s" title="%2$s" rel="designer">%3$s</a>', 'tokokoo' ),
