...
class tokokoo_post_type extends WP_Widget {
	...
	function __construct() {
		...
		$widget_ops = array(
			...
			'description' => __( 'A custom widget to display most recent and popular data from any post type.', 'tokokoo' )
		);
		...
		parent::__construct( 'tokokoo_post_type_widget', __( 'Tokokoo - Post Type', 'tokokoo' ), $widget_ops, $control_ops );
			...
			<ul class="no-list-style">
				...
				<?php foreach( $post_query as $post ) :	setup_postdata( $post ); ?>
					...
					<li class="cl">
						...
						<?php if( $date == true ) { ?>
							<span class="post-time"><?php echo human_time_diff( get_the_time( 'U' ), current_time( 'timestamp' ) ) . __( ' ago', 'tokokoo' ); ?></span>
						<?php } ?>
	...
	?>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'limit' ) ); ?>"><?php _e( 'Limit:', 'tokokoo' ); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'order' ) ); ?>"><?php _e( 'Display:', 'tokokoo' ); ?></label>
			<select class="widefat" name="<?php echo $this->get_field_name( 'order' ); ?>" id="<?php echo $this->get_field_id( 'order' ); ?>">
				<option value="comment_count" <?php selected( $order, 'comment_count' ); ?>><?php _e( 'Popular', 'tokokoo' ); ?></option>
				<option value="date" <?php selected( $order, 'date' ); ?>><?php _e( 'Recent', 'tokokoo' ); ?></option>
			</select>
		</p>
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'excerpt' ) ); ?>"><?php _e( 'Display excerpt ?', 'tokokoo' ); ?></label>
        ...
        <p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'length' ) ); ?>"><?php _e( 'Excerpt length:', 'tokokoo' ); ?></label>
		...
		<?php if( current_theme_supports( 'post-thumbnails' ) && current_theme_supports( 'get-the-image' ) ) { ?>
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'thumb' ) ); ?>"><?php _e( 'Display thumbnail ?', 'tokokoo' ); ?></label>
	        ...
	        <p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'thumb_height' ) ); ?>"><?php _e( 'Thumbnail Size (height x width):', 'tokokoo' ); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'date' ) ); ?>"><?php _e( 'Display date ?', 'tokokoo' ); ?></label>
	      	<input id="<?php echo $this->get_field_id( 'date' ); ?>" name="<?php echo $this->get_field_name( 'date' ); ?>" type="checkbox" value="1" <?php checked( '1', $date ); ?> />&nbsp;
        </p>
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'cat' ) ); ?>"><?php _e( 'Limit to category: ' , 'tokokoo' ); ?></label>
			<?php wp_dropdown_categories( array( 'name' => $this->get_field_name( 'cat' ), 'show_option_all' => __( 'All categories' , 'tokokoo' ), 'hide_empty' => 1, 'hierarchical' => 1, 'selected' => $cat ) ); ?>
		</p>
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('post_type')); ?>"><?php _e( 'Choose the Post Type: ', 'tokokoo' ); ?></label>
