...
if(! class_exists('BFL_Backup')) {
	...
	class BFL_Backup {
		...
		function add_page() {
			$backup_page = add_theme_page( __( 'Backup Options', 'tokokoo' ), __( 'Backup Options', 'tokokoo' ), 'edit_theme_options', 'bfl_backup', array(&$this, 'render_page') );
		}
		...
		public function option_header() {
			...
			$output = '<h2>' . __( 'Theme Options Backup', 'tokokoo' ) . '</h2>';
			$output .= '<div class="updated settings-error below-h2"><p>' . __( 'Note: This is only to backup your Theme Options!', 'tokokoo' ) . '</p></div>';
			return $output;
		}
